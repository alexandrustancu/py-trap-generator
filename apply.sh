...
original_name="py_trap_generator"
original_urlname="py-trap-generator"
original_description="Awesome py_trap_generator created by alexandrustancu" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/py_trap_generator "${name}"
...
then
    ...
    echo "then run 'py_trap_generator' or 'python -m py_trap_generator'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'py_trap_generator' or 'python -m py_trap_generator' or 'poetry run py_trap_generator'"
fi
